...
public:
	...
	~hp_owner() {
		...
		hp->id.store(std::thread::id());//析构会还原回默认值
	}
private:
...
std::atomic<void*>& get_hazard_pointer_for_current_thread() {
	...
	return hazzard.get_pointer();//自定义的类对象以及函数
}
//返回可用的风险指针节点
template<typename T>
...
public:
	...
	bool outstanding_hazard_pointers_for(void* p)//检查是否该节点有风险指针映射
	{
		for (unsigned i = 0; i < max_hazard_pointers; ++i)
		{
	...
	void add_to_reclaim_list(data_to_reclaim* reclaim_node) {
		reclaim_node->next = nodes_to_reclaim.load();//有可能多个线程同时加
